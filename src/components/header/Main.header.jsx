...
function Mainheader() {
  ...
  //hooks views
  const [showMenu, setShowMenu] = React.useState('hidden font-bold w-4/5 h-full  gap-y-8  md:flex md:static  md:flex-row md:gap-4 md:mr-auto  ');
  const [textColor, setTextColor] = React.useState('relative text-quinary py-6 group');

  const handleShowMenu = () => {
    setShowMenu('absolute flex flex-col top-0 left-0 font-bold w-4/5 h-full gap-y-8  md:flex md:static  md:flex-row md:gap-4 md:mr-auto  bg-primary' );
  };
  ...
  return (
    ...
    <header className='flex grow container p-4 md:p-0 gap-8 items-center px-4 bg-primary '>
      {/* ocultar en desktop */}
